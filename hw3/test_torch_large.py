...
# [優化] V100 記憶體夠大，讀取 100 篇歷史能讓模型更懂用戶
MAX_HISTORY_LEN = 100 

# [優化] 單卡 Batch Size (總 Batch = 512 * 8 = 4096)
# 如果遇到 OOM (Out of Memory)，請將此數值改回 256
BATCH_SIZE = 512  

# [調整] 因為難度增加(負採樣變多)，Epoch 稍微減少，避免訓練太久
NUM_EPOCHS = 10 

# [優化] Batch 很大，LR 維持較高水平
LEARNING_RATE = 0.001  

# [關鍵優化] 負採樣比例 1:9 (原本 1:4)
# 利用 8 卡算力強迫模型在更多雜訊中學習，通常能顯著提升 AUC
NUM_NEGATIVES = 9  
...
class NewsEncoder(nn.Module):
    def __init__(self, vocab_size, cat_size, pretrained_emb=None):
        ...
        # 這裡會根據新的 NEWS_DIM (400) + CAT_EMB_DIM (50) 自動調整
...
def main():
    ...
    # 2. 準備資料 (Preprocess) (所有 Rank 都做，確保一致性)
    ...
    if rank == 0:
        ...
        generate_submission(eval_model, test_loader, device, output_path='submission_8gpu_optimized.csv')
